...
public class DsTime_ImportExportVarRicFromCSV : BaseNetLogic
{
	...
	string[] FileHeader = { "PercorsoDriverComm(su Qstudio)", "NomeStruct", "NomeTagPLC", "IndiceArray", "NomeTagHmi", "TipoTag", "NomeObjectL1", "ObjectTypeL1", "NomeObjectL2", "ObjectTypeL2", "NomeObjectL32", "ObjectTypeL3" };
	...
	public void ImportaTag()
	{
		...
		try
		{
			...
			for (Row = 1; Row < fileLines.Count; ++Row)       //Salto la prima riga che è quella di intestazione
			{
				...
				if (!string.IsNullOrEmpty(NomeTagHmi) && !string.IsNullOrWhiteSpace(NomeTagHmi)) //se la riga è vuota salto alla successiva
				{
					...
					if (LogicObject.GetVariable("CreaCollegamentiDinamici").Value && !string.IsNullOrEmpty(DriverRef) && !string.IsNullOrWhiteSpace(DriverRef))
					{
		...
		foreach (var item in NodoPadre.GetNodesByType<IUAVariable>())
		{
			...
			if (DynLink is not null)
			{
				...
				row[PercorsoDriverCommIndex] = $"{DynamicLinkString[0]}/{DynamicLinkString[1]}/{DynamicLinkString[2]}";     //Percorso Driver Comm
																															//Project.Current.GetVariable(item.Children.GetVariable("DynamicLink").Value.Value.ToString()).Owner.Equals(QPlatform.CommunicationDriver.TagStructure);
				row[5] =
	...
	private char? CheckCharacterSeparator(string separator)
	{
		...
		else if (char.TryParse(separator, out char result))
			return result;
		return null;
	}
}

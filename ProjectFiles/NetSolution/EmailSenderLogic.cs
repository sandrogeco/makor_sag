...
{
    //private string mMailSubject, mMailBody, mAttachmentfile;
    //NodeId mMailingListNode;
    ...
    [ExportMethod]
    public void SendEmail_LongRunningTask(string mailSubject, string mailBody, string Attachmentfile = "", NodeId MailingListNode = null)
    {
        //mMailSubject = mailSubject;
        //mMailBody = mailBody;
        //mAttachmentfile = Attachmentfile;
        NodeId mMailingListNode = MailingListNode is null ? LogicObject.GetObject("MailingList").NodeId : MailingListNode;

        LongRunningTask mytask = new(() => SendEmail(mailSubject, mailBody, Attachmentfile, mMailingListNode), LogicObject);
        mytask.Start();
    }
    ...
    ///// 
    ///// </summary>
    ///// <param name="mailSubject">Testo da inserire nel oggetto della mail</param>
    ///// <param name="mailBody">Testo da inserire nel corpo della mail</param>
    ///// <param name="Attachmentfile">Percorso dell'eventuale file allegato da inviare</param>
    ///// <param name="MailingListNode">Specificare una lista differente se non si vuole usare quella già predisposta per lo script</param>    
    private void SendEmail(string mailSubject, string mailBody, string Attachmentfile = "", NodeId MailingListNode = null)
    {
        //private void SendEmail()
        //{
        //string mailSubject = mMailSubject;
        //string mailBody = mMailBody;
        //string Attachmentfile = mAttachmentfile;
        //NodeId MailingListNode = mMailingListNode;

        // Creazione lista degli utenti a cui inviare la mail
        IUANode MailingList = LogicObject.Context.GetNode(MailingListNode);

        if (MailingList.Children.Count == 0)
            return;

        string mailToAddress = "";
        foreach (var utente in MailingList.GetNodesByType<IUAVariable>())
        {
            mailToAddress = utente.Value;
            break;
        }
        ...
        var fromAddress = new MailAddress(senderAddress, "From Makor");
        ...
        var message = CreateEmailMessage(fromAddress, toAddress, mailBody, mailSubject, Attachmentfile);

        //Inserirsco gli altri utenti nella lista NOTA: salto il primo xk è già stato aggiunto
        foreach (var utente in MailingList.GetNodesByType<IUAVariable>().Skip(1))
            message.To.Add(new MailAddress(address: utente.Value, displayName: utente.BrowseName));  // The MailMessage.To property is a collection of emails, so you can add different recipients using: 

        TrySendEmail(message);
    }

    private MailMessageWithRetries CreateEmailMessage(MailAddress fromAddress, MailAddress toAddress, string mailBody, string mailSubject, string attachment)
    {
        ...
        if (!string.IsNullOrEmpty(attachment))
        {
            ...
            Attachment data = new(attachment);  //@"C:\ALavori\Works\In Progress\Supervisore_Neuhofer\Support\MakorLineProcessDataReport.csv";            
    ...
    {
        senderAddress = GetVariableValue("SenderEmailAddress").Value;
